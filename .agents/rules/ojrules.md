...
| Content | MDX (local files, next-mdx-remote + gray-matter) |
...
components/
  ...
  mdx/             → MDX custom components for content rendering
  sections/        → Page-specific sections organized by route (home/, work/, etc.)
content/
  projects/        → Project MDX files
  journal/         → Blog MDX files
lib/
  utils.ts         → cn(), formatDate, calculateReadingTime
  mdx.ts           → getAllProjects, getProjectBySlug, getAllPosts, getPostBySlug
...
- Parse with `next-mdx-remote/rsc` (server-side).
- Frontmatter via `gray-matter`, validated against Zod schemas.
- Reading time: word count ÷ 200 wpm.
- Custom components registered in `mdx-components.tsx`.
- All MDX rendering in Server Components.
